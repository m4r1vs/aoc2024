...
fn is_ordering_correct(page_ordering: &[usize], rules: &HashMap<usize, Vec<usize>>) -> bool {
    for (i, page) in page_ordering.iter().enumerate() {
        for j in (i + 1)..(page_ordering.len() - 1) {
            if let Some(rule) = rules.get(page) {
                if rule.contains(&page_ordering[j]) {
                    return false;
                }
            }
        }
...
fn fix_ordering(page_ordering: &Vec<usize>, rules: &HashMap<usize, Vec<usize>>) -> Vec<usize> {
    let mut fixed_ordering: Vec<usize> = page_ordering.clone();

    for (i, page) in page_ordering.iter().enumerate() {
        for j in (i + 1)..(page_ordering.len() - 1) {
            if let Some(rule) = rules.get(page) {
                if rule.contains(&page_ordering[j]) {
                    (fixed_ordering[i], fixed_ordering[j]) = (fixed_ordering[j], fixed_ordering[i]);
                    if !is_ordering_correct(&fixed_ordering, rules) {
                        return fix_ordering(&fixed_ordering, rules);
                    } else {
